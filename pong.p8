...
-- variables that survive across games
player_points = 0
com_points = 0
scored = ""
...
function _update()
	-- player controls
	if btn(⬆️) and flr(player.y) > court_top + 1 then
		player.y -= player.speed
	end

	if btn(⬇️) and flr(player.y) + player.h < court_bottom - 1 then
		player.y += player.speed
	end
	...
	and ball.y >= com.y
	and ball.y + ball.w <= com.y + com.h then
...
function _draw()
	...
	-- ball
	rectfill(ball.x, ball.y, ball.x + ball.w, ball.y + ball.w, ball.c)

	-- player
	rectfill(player.x, player.y, player.x + player.w, player.y + player.h, player.c)

	-- computer
	rectfill(com.x, com.y, com.x + com.w, com.y + com.h, com.c)
